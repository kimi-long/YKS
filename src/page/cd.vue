    ...
    <div id="calendar" :class="{'change':isChange}">
        ...
        <div class="year-month">
            <div class="year-month_left">
                ...
                <span class="leftyear">{{currentYear}}年<span style="margin-left: 10px"> {{currentMonth}}月 </span><span style="margin-left: 10px"> {{otherDay}}日 </span></span>
                <!--<span class="leftmonth"></span>-->
                <span class="leftweek">周{{weekdays[newWeek]}}</span>
                <!--<ul>-->
                    <!--<li>{{currentYear}}年</li>-->
                    <!--<li>{{currentMonth}}月</li>-->
    ...
    export default {
        ...
        methods: {
            ...
            otherMonth(day){
                this.otherDay = day;
                if(day<15){
                    this.rightSliding(day)
                }else if(day>15){
                    this.leftSliding(day)
                }
            },
            addSchedule(){
                this.$emit('add')
            ...
            rightSliding(day){
                const self = this;
                self.pickNext(self.currentYear, self.currentMonth,day);
                self.fadeOut = true;
                self.fadeIn = false;
                self.fadeInR = false;
                self.fadeOutR = false;
            ...
            leftSliding(day){
                const self = this;
                self.pickPre(self.currentYear, self.currentMonth,day);
                self.fadeOutR = true;
                self.fadeInR = false;
                self.fadeOut = false;
                self.fadeIn = false;
            ...
            //     上个月信息
            pickPre(year, month,day) {
                const d = new Date(this.formatDate(year, month, day));
                d.setDate(0);
                this.initData(this.formatDate(d.getFullYear(), d.getMonth() + 1, 1));
            },
            // 下个月信息
            pickNext(year, month,day) {
                const d = new Date(this.formatDate(year, month, day));
                d.setDate(35);
                this.initData(this.formatDate(d.getFullYear(), d.getMonth() + 1, 1));
            },

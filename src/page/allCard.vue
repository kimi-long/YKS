        ...
        <div  v-loading="loading" element-loading-custom-class="loading_color" element-loading-spinner="el-icon-loading">
            <div class="ThemedWrapper-jd76ve-0 hnMLRX slide pop-enter-done">
                <div class="InfiniteScroller__ScrollerWrapper-x1lcuc-0 hMVrB" style="height: 100vh; padding-top: 8px;">
                    <a class="CardTemplateIndex__CardTemplateWrapper-sc-14214yf-0 kPDrbV" v-for="(item,index) in cardList" @click="tocard_detail(item.ID)"
                    >
...
<!--                                </div>-->
                            ...
                            <section class="NewCardTemplateComponent__Wrapper-sc-18n2c0i-0 cWMxiU default-cardTemplate" :style="{backgroundImage:'url(' + item.CradImgURL + ')', backgroundRepeat:'no-repeat'}"v-else>
                                <div class="inner-wrapper">
                                    <div class="card-information">
                                        <div class="card-name fontBold" v-if="item.CardType == 1">{{item.yxdays}}天{{item.Kynumber}}次</div>
                                        <div class="card-name fontBold" v-if="item.CardType == 2">{{item.Kynumber}}次不限期</div>
                                        <div class="card-name fontBold" v-if="item.CardType == 3">{{item.yxdays}}天不限次</div>
                            ...
                            <div class="CardTemplateComponent__CardTemplateTextWrapper-sc-1slov8k-2 jzSyzs">
                                ...
                                <span class="CardTemplateComponent__CardTemplatePrice-sc-1slov8k-1 dkRCIX" v-if="item.CardType == 4">￥{{item.SellingPrice}}</span>
                                <span class="CardTemplateComponent__CardTemplatePrice-sc-1slov8k-1 dkRCIX" v-else>￥{{item.SellingPrice}}</span>
                            </div>
                        </div>
                    </a>
...
<script>
    import {getListMemberCard} from  '../api/getData'
    ...
    export default {
        ...
        methods: {
            ...
            async getListMemberCard() {
                // debugger;
                this.loading = true;
                const crs = await getListMemberCard({
